...
app.post('/auth', function(request, response) {
	...
	// Ensure the input fields exists and are not empty
        if (username && password) {
            ...
            conn.query('SELECT * FROM '+tableName+' WHERE '+email+' = ? AND password = ?', [username, password], function(error, results, fields) {
                ...
                if (results.length > 0) {
                    ...
                    request.session.code = (isStudent? results[0].matricule : results[0].code_prof)
    ...
    form.parse(req, function (err, fields, files) {
      ...
      conn.query(sql, async function (err, result) {
        ...
        if(files && newID){
          ...
          /*INSERT FILES*/
          var sqlFiles = "INSERT INTO Files (name, types, parentId, createdBy, lastModifiedBy) VALUES ?";
          var values = [];
          files.forEach(element => {
            values.push([element.originalFilename, path.extname(element.originalFilename), newID, createdBy, createdBy]);
